...
export default function CustomerAppointments() {
  ...
  const payMutation = useMutation({
    mutationFn: async (id: string) => {
      const { data } = await paymentAPI.createIntent(id);
      await paymentAPI.confirm(id, data.data.clientSecret);
    },
    onSuccess: () => {
        ...
        <div className="space-y-3">
          {appointments.map(apt => (
            <div key={apt._id} className="card p-0 overflow-hidden">
              <div className="p-4 cursor-pointer" onClick={() => setExpanded(expanded === apt._id ? null : apt._id)}>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <p className="font-semibold text-slate-800">{apt.services?.map(s => s.serviceType).join(', ')}</p>
                      {statusBadge(apt.status)}
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${apt.priority === 'emergency' ? 'bg-red-100 text-red-700' : apt.priority === 'urgent' ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-600'}`}>
                        {apt.priority}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    {apt.status === 'completed' && apt.paymentStatus !== 'paid' && (
                      <button
                        onClick={e => { e.stopPropagation(); payMutation.mutate(apt._id); }}
